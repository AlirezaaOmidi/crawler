                ...
                for colu in prices2.index:
                    try:
                        if ((abs((prices2[f"{colu}"] - now_mean_zero) / now_mean_zero * 100) > primary_alarm_treshold)
                                or (abs((week_mean_copy[
                                             f"{colu}"] - days_history_week_mean) / days_history_week_mean * 100) > primary_alarm_treshold)
                                or (abs((month_mean_copy[
                                             f"{colu}"] - days_history_month_mean) / days_history_month_mean * 100) > primary_alarm_treshold)
                                or (abs((days_history_24_copy[
                                             f"{colu}"] - days_history_24mean) / days_history_24mean * 100) > primary_alarm_treshold)
                                or (abs((hours_history_1_copy[
                                             f"{colu}"] - hours_history_1mean) / hours_history_1mean * 100) > primary_alarm_treshold)):
                            prices2[f"{colu}"] = ""
                            week_mean[f"{colu}"] = ""
                            month_mean[f"{colu}"] = ""
            ...
            try:
                    try:
                        if ((abs((prices2[f"{colu}"] - now_mean_zero) / now_mean_zero * 100) > secondary_alarm_treshold)
                                or (abs((week_mean_copy[
                                             f"{colu}"] - days_history_week_mean) / days_history_week_mean * 100) > secondary_alarm_treshold)
                                or (abs((month_mean_copy[
                                             f"{colu}"] - days_history_month_mean) / days_history_month_mean * 100) > secondary_alarm_treshold)
                                or (abs((days_history_24_copy[
                                             f"{colu}"] - days_history_24mean) / days_history_24mean * 100) > secondary_alarm_treshold)
                                or (abs((hours_history_1_copy[
                                             f"{colu}"] - hours_history_1mean) / hours_history_1mean * 100) > secondary_alarm_treshold)):
                            prices2[f"{colu}"] = ""
                            week_mean[f"{colu}"] = ""
                            month_mean[f"{colu}"] = ""
                            days_history_24[f"{colu}"] = ""
